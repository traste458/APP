...
public partial class ReporteTramite_ReporteTramiteCP : System.Web.UI.Page
{
    ...
    public DataTable dtReporte { get { return (DataTable)Session["ResultConsultaAvanzada"]; } set { Session["ResultConsultaAvanzada"] = value; } }
    private string _sectores = "";
    #endregion
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            //    }
            //}
            if (Page.Request.Params["dato"] != null)
            {
                this.textSearch.Text = Page.Request.Params["dato"].ToString();
                ScriptManager.RegisterStartupScript(this, this.GetType(), this.Page.Title, "EjecutarConsulta()", true);
            }
            ...
            CargarAutoridades( );      
            CargarSector();
            this.txtFechaInicial.Text = DateTime.Today.AddMonths(-1).ToString("dd/MM/yyyy");
            this.txtFechaFinal.Text = DateTime.Today.ToString("dd/MM/yyyy");
            #endregion
        }
    }
        ...
        try
        {
            if (lbEntryDataType == "documentos")
            {
                if (lbRuta != "")
                    ...
                    Session["Ruta"] = lbPath;
                scrRespuesta = this.Request.Url.AbsoluteUri.ToString().Replace("ReporteTramiteCP.aspx", "DescargarDocumentos.aspx");
                SMLog.Escribir(Severidad.Critico, this.Page.Title + "::DocumentosVITAL :: " + lbRuta + " :: "+ lbPath);
            }
    ...
    {
        this.ods_buscador.Select();
        //TODO: JSACOSTAV 20220323
        ScriptManager.RegisterStartupScript(this, this.GetType(), this.Page.Title, "ocultarMalla()", true);
    }
    private List<SILPA.AccesoDatos.ConsultaPublica.ConsultaPublicaEntity> IniciarBusqueda(string parametroBusqueda, string tipoBusqueda)
    {
        try
        {
            Dictionary<string, object> row = new Dictionary<string, object>();
            ConsultaPublica _consultaPublica = new SILPA.LogicaNegocio.ConsultaPublica.ConsultaPublica();
            List<SILPA.AccesoDatos.ConsultaPublica.ConsultaPublicaEntity> lst;
            lst = _consultaPublica.BuscarCampoPaginado(parametroBusqueda, -1, -1, tipoBusqueda);
            return lst;
        }
        catch (Exception ex)
        {
    ...
    /// <param name="e"></param>
    protected void RadResultadosBuscador_PageIndexChanged(object sender, Telerik.Web.UI.GridPageChangedEventArgs e)
    {
        this.RadResultadosBuscador.DataSource = IniciarBusqueda(this.textSearch.Text.Trim(), this.ddlTipoBusqueda.SelectedValue);
        this.RadResultadosBuscador.DataBind();
    }
    /// <summary>
    /// Evento que se ejecuta al interacturar con un registro de la grilla
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void RadResultadosBuscador_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
    {
        if (e.CommandName == RadGrid.FilterCommandName)
        {
            Pair filterPair = (Pair)e.CommandArgument;
            TextBox filterBox = (e.Item as GridFilteringItem)[filterPair.Second.ToString()].Controls[0] as TextBox;
            //RadResultadosBuscador.MasterTableView.FilterExpression = "[" + filterPair.Second + "] LIKE '%" + filterBox.Text + "%'";
            //RadResultadosBuscador.Rebind();
        }
    }
    
    protected void ods_buscador_ObjectCreating(object sender, ObjectDataSourceEventArgs e)
    {
        this.dvBusquedaAvanzada.Visible = false;
        this.dvResultados.Visible = false;
        switch (this.ddlTipoBusqueda.SelectedValue)
        {
            case "BusquedaAvanzada":
                this.dvBusquedaAvanzada.Visible = true;
                break;
            default:
                e.ObjectInstance = new SILPA.LogicaNegocio.ConsultaPublica.ConsultaPublica();
                this.dvResultados.Visible = true;
                this.textSearch.Enabled = true;
                this.imgBtnBuscar.Enabled = true;
                break;
        }
        
    }
    protected void ods_buscador_Load(object sender, EventArgs e)
    {

    }
    protected void ods_buscador_Selected(object sender, ObjectDataSourceStatusEventArgs e)
    {

    }
    

    #endregion ModificacionBuscador
    ...
    protected void ddlTipoBusqueda_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        this.imgBtnBuscar.Enabled = false;
        switch (this.ddlTipoBusqueda.SelectedValue)
        {
            case "BusquedaAvanzada":
                this.dvBusquedaAvanzada.Visible = true;
    ...
    protected void btnAddSector_Click(object sender, ImageClickEventArgs e)
    {
    ...
    protected void btnElimSector_Click(object sender, ImageClickEventArgs e)
    {
    ...
    /// <summary>
    /// Funcion que carga el combo de las autoridades ambientales
    /// </summary>
    ...
    /// <summary>
    /// Funcion que Carga el combo de sector
    /// </summary>
    ...
    /// <summary>
    /// Web Method que obtiene la lista de los solicitante que cumplan con el valor especificado
    /// </summary>
    /// <param name="p_strSolicitante">string con el nombre del solicitante</param>
    /// <returns>List con la informacion de las solicitantes</returns>
    [WebMethod]
    public static List<string> GetSolicitante(string p_strSolicitante)
    {
        List<string> objLstListaOpciones = new List<string>();

        SILPA.LogicaNegocio.Generico.Persona personas = new SILPA.LogicaNegocio.Generico.Persona();
        DataTable dat = personas.ConsultarPersonasActivas(p_strSolicitante);

        if (dat != null && dat.Rows.Count > 0)
        {

            objLstListaOpciones = dat.AsEnumerable().Select(datos => datos.Field<string>("PER_NOMBRE_COMPLETO") + "@/" + datos.Field<int>("PER_ID")).ToList();
            objLstListaOpciones.OrderBy(dato => dato);

            //Verificar si se obtuvo la información
            if (objLstListaOpciones != null && objLstListaOpciones.Count > 0)
            {
                //Recortar el número de resultados a mostrar
                if (objLstListaOpciones.Count > 10)
                {
                    objLstListaOpciones.RemoveRange(10, objLstListaOpciones.Count - 10);

                    //Adicionar opción informativa
                    objLstListaOpciones.Add("...@/-1");
                }
            }

        }

        return objLstListaOpciones;
    }
    ...
    protected void btn_buscar_Click(object sender, EventArgs e)
    {
        if ((txtFechaInicial.Text == "") || (txtFechaFinal.Text == ""))
        {
            ...
            grdReporte.MasterTableView.NoMasterRecordsText = "Se debe ingresar el filtro de fecha desde - hasta.";
            grdReporte.DataBind();
    ...
    private void Consultar()
    {
        try
        {
            ...
            int departamentoID = -1, municipioID = -1;
            ...
            municipioID = Convert.ToInt32(cboMunicipio.SelectedValue != string.Empty ? cboMunicipio.SelectedValue : "-1");
            departamentoID = Convert.ToInt32(cboDepartamento.SelectedValue != string.Empty ? cboDepartamento.SelectedValue : "-1");
            ...
            if (_sectores.Equals(""))
            {
                ...
                txtNombreProyecto.Text.Trim(), departamentoID, municipioID, TxtNumeroExpediente.Text.Trim(), "", CadenaHidrografia(), int.Parse(idUsuario.ToString()), this.cboEstadoResolucion.SelectedValue, this.cboEstadoTramite.SelectedValue, this.hdfSolicitanteID.Value);
            }
            else
            {
                ...
                dtReporte = objTramite.ListarTramitesRPT(false, fechaInicial, fechaFinal,
                Convert.ToInt32(ddlTipoTramite.SelectedValue), TxtSilpaNumero.Text.Trim(), Convert.ToInt32(cboAutoridadAmbiental.SelectedValue),
                txtNombreProyecto.Text.Trim(), departamentoID, municipioID, TxtNumeroExpediente.Text.Trim(), _sectores, CadenaHidrografia(), int.Parse(idUsuario.ToString()), this.cboEstadoResolucion.SelectedValue, this.cboEstadoTramite.SelectedValue, this.hdfSolicitanteID.Value);
            }
            grdReporte.DataSource = dtReporte;
            grdReporte.MasterTableView.NoMasterRecordsText = "No se han encontrado Registros.";
            grdReporte.DataBind();
            grdReporte.Visible = true;
    ...
    protected void grdReporte_PageIndexChanged(object sender, GridPageChangedEventArgs e)
    {
        try
        {
            this.grdReporte.CurrentPageIndex = e.NewPageIndex;
            this.grdReporte.DataSource = dtReporte;
    ...
    /// <summary>
    /// Funcion que limpia las opciones del buscador avanzado
    /// </summary>
    protected void LimpiarBuscadrAvanzado() 
    {
        this.txtNombreProyecto.Text = string.Empty;
        this.TxtNumeroExpediente.Text = string.Empty;
        this.TxtSilpaNumero.Text = string.Empty;
        this.cboAutoridadAmbiental.SelectedIndex = 0;
        this.cboEstadoResolucion.SelectedIndex = 0;
        this.cboEstadoTramite.SelectedIndex = 0;
        this.txtFechaInicial.Text = DateTime.Today.AddMonths(-1).ToString("dd/MM/yyyy");
        this.txtFechaFinal.Text = DateTime.Today.ToString("dd/MM/yyyy");
        this.cboDepartamento.SelectedIndex = 0;
        Utilidades.LlenarComboVacio(this.cboMunicipio);
        this.CboArea.SelectedIndex = 0;
        Utilidades.LlenarComboVacio(this.CboZona);
        Utilidades.LlenarComboVacio(this.CboSubZona);
        this.cboSectores.SelectedIndex = 0;
        this.lstSectorSel.Items.Clear();
        this.dtReporte = null;
        this.grdReporte.CurrentPageIndex = 0;
        this.grdReporte.DataSource = this.dtReporte;
        this.grdReporte.DataBind();
    ...
    {
        LimpiarBuscadrAvanzado();
    }
    protected void grdReporte_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            Label lblAutoridadAmb = (Label)e.Item.FindControl("lblAutoridadAmb");
            Image imgAutoridadAmb = (Image)e.Item.FindControl("imgAutoridadAmb");
            LinkButton lbVerDetalles = (LinkButton)e.Item.FindControl("lbVerDetalles");
            string strNombreAutoridad = lblAutoridadAmb.Text.Trim().Replace(" ", "");
            if (strNombreAutoridad == "ANLA")
            {
                imgAutoridadAmb.ImageUrl = @"~/App_Themes/Img/Autoridades/anla.png";
            }
            else if (strNombreAutoridad == "" || strNombreAutoridad == "ANLA")
            {
                imgAutoridadAmb.ImageUrl = @"~/App_Themes/Img/Autoridades/anla.png";
            }
            else if (strNombreAutoridad == "MADS")
                imgAutoridadAmb.ImageUrl = @"~/App_Themes/Img/logoEntidad.png";
            else if (strNombreAutoridad != "")
                imgAutoridadAmb.ImageUrl = @"~/App_Themes/Img/Autoridades/" + strNombreAutoridad + ".jpg";
            else if (strNombreAutoridad == "VITAL")
                imgAutoridadAmb.ImageUrl = @"~/App_Themes/Img/LogoVital.png";
            else if (strNombreAutoridad == "SILA" || strNombreAutoridad == "SILAMC")
                imgAutoridadAmb.ImageUrl = @"~/App_Themes/Img/Autoridades/anla.png";

            ScriptManager.GetCurrent(this).RegisterAsyncPostBackControl(lbVerDetalles);
        }


    }

    protected void grdReporte_ItemCommand(object sender, GridCommandEventArgs e)
    {
        if (e.CommandName == "DETALLE")
        {
            int index = Convert.ToInt32(e.CommandArgument);
            Label lbNumeroSilpa = (Label)this.grdReporte.Items[index].FindControl("lbNumeroSilpa");
            Label lbIdSol = (Label)this.grdReporte.Items[index].FindControl("lblIdSol");
            Label lblExpCod = (Label)this.grdReporte.Items[index].FindControl("lblExpCod");
            Label lblTipoTramite = (Label)this.grdReporte.Items[index].FindControl("lblTipoTramite");
            if (idUsuario != 0)
            {
                if (lblTipoTramite.Text == "Reconocimiento como Tercero Interviniente")
                {
                    if (lblExpCod.Text != "")
                    {
                        ...
                        ScriptManager.RegisterStartupScript(this,this.GetType(), "OpenWindow", "window.open('ReporteTramiteDos.aspx ? NSilpa ="  + lbNumeroSilpa.Text + " & Id = " + lbIdSol.Text + " & Exp = " + exp + "','_newtab');", true);
                    }
                    else
                    {
                        ScriptManager.RegisterStartupScript(this,this.GetType(), "OpenWindow", "window.open('ReporteTramiteDos.aspx?NSilpa=" + lbNumeroSilpa.Text + "&Id=" + lbIdSol.Text+ "','_newtab');",true);
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "OpenWindow", "window.open('ReporteTramiteDetallesCiudadano.aspx?NSilpa=" + lbNumeroSilpa.Text + "&Id=" + lbIdSol.Text + "&Ubic=Ext','_newtab');", true);
            }
        }
    }
