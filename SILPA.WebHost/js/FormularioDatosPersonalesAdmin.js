...
$(document).ready(function () {
    ...
    $('#txtNumeroIdentificacion, #txtNumeroDocumentoJuridica, #txtNumeroDocumentoPrivada').focusout(function () {
        if ($('#txtNumeroIdentificacion').val().length > 5 || $('#txtNumeroDocumentoJuridica').val().length > 5 || $('#txtNumeroDocumentoPrivada').val().length > 5) {
...
function CargarDatosAutoridadAmbiental() {
    $.ajax({
        async: false,
        type: "POST",
        url: "FormularioDatosPersonalesAdmin.aspx",
...
function CargarInformacionRegistrado(numeroDocumento, tipoDocumento) {
    $.ajax({
        async: false,
        type: "POST",
        url: "FormularioDatosPersonalesAdmin.aspx",
        data: { Accion: "PersonaRegistrada", numeroIdentificacion: numeroDocumento, tipoIdentificacion: tipoDocumento },
        dataType: "json",
        success: function (data) {
            if (data !== undefined) {
...
function LlenarUsuarioRegistrado(datos) {
    if (datos !== null && datos.length > 0) {
        ...
        if (datos[0].CodigoTipoPersona == 1) {
            ...
            $("#selPaisNatural").val(datos[0].DirCorrespondenciaPaisId).change();
            $("#selDepartamentoNatural").val(datos[0].DirCorrespondenciaDepartamentoId).change();
            $("#selMunicipioNatural").val(datos[0].DirCorrespondenciaMunicipioId).change();
            $("#selCorregimientoNatural").val(datos[0].DirCorrespondenciaCorregimientoId).change();
            $("#selVeredaNatural").val(datos[0].DirCorrespondenciaVeredaId).change();
            $("#txtDireccionNatural").val(datos[0].DirCorrespondenciaDireccionPersona);
        }
        else if (datos[0].CodigoTipoPersona == 2) {
            $("#divPersonaJuridicaPublica").css("display", "block");
            $("#divPersonaNatural").css("display", "none");
            $("#divPersonaJuridicaPrivada").css("display", "none");
            var rbJuridicaPublica = document.getElementById('juridicaPublica');
            rbJuridicaPublica.checked = true;
            tipoUsuarioSeleccionado = "juridicaPublica";
            $("#txtRazonSocialJuridica").val(datos[0].RazonSocial);
            $("#selTipoDocumentoJuridica").val(datos[0].TipoDocumentoIdentificacion).change();
            $("#txtNumeroDocumentoJuridica").val(datos[0].NumeroIdentificacion);
            $("#txtTelefonoJuridica").val(datos[0].Telefono);
            $("#txtCelularJuridica").val(datos[0].Celular);
            $("#txtFaxJuridica").val(datos[0].Fax);
            $("#txtCorreoJuridica").val(datos[0].CorreoElectronico);
            $("#selPaisJuridica").val(datos[0].DirCorrespondenciaPaisId).change();
            $("#selDepartamentoJuridica").val(datos[0].DirCorrespondenciaDepartamentoId).change();
            $("#selMunicipioJuridica").val(datos[0].DirCorrespondenciaMunicipioId).change();
            $("#selCorregimientoJuridica").val(datos[0].DirCorrespondenciaCorregimientoId).change();
            $("#selVeredaJuridica").val(datos[0].DirCorrespondenciaVeredaId).change();
            $("#txtDireccionJuridica").val(datos[0].DirCorrespondenciaDireccionPersona);
        }
        else if (datos[0].CodigoTipoPersona == 3) {
            var rbJuridicaPrivada = document.getElementById('juridicaPrivada');
            rbJuridicaPrivada.checked = true;
            $("#divPersonaJuridicaPrivada").css("display", "block");
            $("#divPersonaNatural").css("display", "none");
            $("#divPersonaJuridicaPublica").css("display", "none");
            tipoUsuarioSeleccionado = "juridicaPrivada";
            $("#divPersonaJuridicaPrivada").css("display", "block");
            $("#txtRazonSocialPrivada").val(datos[0].RazonSocial);
            $("#selTipoDocumentoPrivada").val(datos[0].TipoDocumentoIdentificacion).change();
            $("#txtNumeroDocumentoPrivada").val(datos[0].NumeroIdentificacion);
            $("#txtTelefonoPrivada").val(datos[0].Telefono);
            $("#txtCelularPrivada").val(datos[0].Celular);
            $("#txtFaxPrivada").val(datos[0].Fax);
            $("#txtCorreoPrivada").val(datos[0].CorreoElectronico);
            $("#selPaisPrivada").val(datos[0].DirCorrespondenciaPaisId).change();
            $("#selDepartamentoPrivada").val(datos[0].DirCorrespondenciaDepartamentoId).change();
            $("#selMunicipioPrivada").val(datos[0].DirCorrespondenciaMunicipioId).change();
            $("#selCorregimientoPrivada").val(datos[0].DirCorrespondenciaCorregimientoId).change();
            $("#selVeredaPrivada").val(datos[0].DirCorrespondenciaVeredaId).change();
            $("#txtDireccionPrivada").val(datos[0].DirCorrespondenciaDireccionPersona);
        }
...
function AdicionarContacto() {
    ...
    if (contar == 0) {
        $('#divListaContacto').append('<div id="' + divTipoContacto + '" class="panel-title"><span class="col-md-1"><label class="form-control-xsm" name="' + document.getElementById('selContacto').value + '"> LISTA DE USUARIOS DE: ' + $("#selContacto :selected").text() + '</label></span></div>');
    }
    /////////////////////////////////////////////////////////////////   div encabezado contacto comunicaciones 
    ...
    $("#divDinamico input").each(function (index) {
...
function CancelarAdicionarContacto() {
    $("#selContacto").val("0").change();
}
...
function EnviarFormulario() {
    if (ValidarDatos()) {
        ...
        if (ver == -1) {
            ...
            //    matchesListas = document.querySelectorAll("#divListaContacto");
            valorCampoContacto = {}
            var numero = 0;
            for (var i = 0, l = matches.length; i < l; i++) {
...
function VerificarExistenciaPorNumeroIdentificacion() {
    ...
    var tipoDocumento = "";
    if (rbvalue[0].id == "Natural" && rbvalue[0].checked == true)
    { numeroDocumento = $("#txtNumeroIdentificacion").val(); tipoDocumento = document.getElementById('slTipoDocumento').value; }
    else if (rbvalue[1].id == "juridicaPublica" && rbvalue[1].checked == true)
    { numeroDocumento = $("#txtNumeroDocumentoJuridica").val(); tipoDocumento = document.getElementById('selTipoDocumentoJuridica').value; }
    else if (rbvalue[2].id == "juridicaPrivada" && rbvalue[2].checked == true)
    { numeroDocumento = $("#txtNumeroDocumentoPrivada").val(); tipoDocumento = document.getElementById('selTipoDocumentoJuridica').value; }

    var retorno = "";
    if (numeroDocumento != null && numeroDocumento.length > 1) {
        $.ajax({
            async: false,
            type: "POST",
            url: "FormularioDatosPersonalesAdmin.aspx",
            data: { Accion: "VerificarNumeroIdentificacion", numeroIdentificacion: numeroDocumento },
            dataType: "json",
            success: function (data) {
                if (data !== undefined) {
                    switch (data) {
                        case -1:
                            $("#divErrores").append('<label class="form-control-xsm">No existe el USUARIO INGRESADO con ese número de identificación</label>');
                            $("#divErrores").css("display", "block");
                            $("#btnEnviar").show();
                            break;
                        case 0:
                            CargarInformacionRegistrado(numeroDocumento, tipoDocumento);
                            $("#btnEnviar").hide();
                            break;
                        case 1:
...
function LimpiarFormulario() {
    ...
    $("#txtNumeroDocumentoJuridica").val("");
    $("#txtDireccionJuridica").val("");
    $("#txtTelefonoJuridica").val("");
    $("#txtCelularJuridica").val("");
    $("#txtFaxJuridica").val("");
    $("#txtCorreoJuridica").val("");
    $("#selTipoDocumentoJuridica, #selDepartamentoJuridica, #selMunicipioJuridica").val("0").change();
    $("#txtRazonSocialPrivada").val("");
    $("#txtNumeroDocumentoPrivada").val("");
    $("#txtDireccionPrivada").val("");
    $("#txtTelefonoPrivada").val("");
    $("#txtCelularPrivada").val("");
    $("#txtFaxPrivada").val("");
    $("#txtCorreoPrivada").val("");
    $("#selTipoDocumentoPrivada, #selDepartamentoPrivada, #selMunicipioPrivada").val("0").change();
}
function Actualizar() {
    if (ValidarDatos()) {
        var personaRegistro = "";
        switch (tipoUsuarioSeleccionado) {
            case "Natural":
                ...
                break;
            case "juridicaPublica":
                var veredaPublica = 0; if (document.getElementById('selVeredaPrivada').value != "") { vereda = document.getElementById('selVeredaPrivada').value; };
                personaRegistro =
                    {
                        "RazonSocialJuridica": $("#txtRazonSocialJuridica").val(),
                        "NumeroDocumentoJuridica": $("#txtNumeroDocumentoJuridica").val(),
                ...
                break;
            case "juridicaPrivada":
                var veredaPrivada = 0; if(document.getElementById('selVeredaPrivada').value !=""){vereda = document.getElementById('selVeredaPrivada').value;};
                personaRegistro =
                    {
                        "RazonSocialPrivada": $("#txtRazonSocialPrivada").val(),
                        "NumeroDocumentoPrivada": $("#txtNumeroDocumentoPrivada").val(),
